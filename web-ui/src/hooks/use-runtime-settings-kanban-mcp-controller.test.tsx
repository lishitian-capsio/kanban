...
} from "@/hooks/use-runtime-settings-kanban-mcp-controller";
import type { RuntimeAgentId, RuntimeKanbanMcpServer, RuntimeKanbanMcpServerAuthStatus } from "@/runtime/types";

const fetchKanbanMcpSettingsMock = vi.hoisted(() => vi.fn());
const fetchKanbanMcpAuthStatusesMock = vi.hoisted(() => vi.fn());
const runKanbanMcpServerOAuthMock = vi.hoisted(() => vi.fn());
const saveKanbanMcpSettingsMock = vi.hoisted(() => vi.fn());

vi.mock("@/runtime/runtime-config-query", () => ({
	fetchKanbanMcpAuthStatuses: fetchKanbanMcpAuthStatusesMock,
	fetchKanbanMcpSettings: fetchKanbanMcpSettingsMock,
	runKanbanMcpServerOAuth: runKanbanMcpServerOAuthMock,
	saveKanbanMcpSettings: saveKanbanMcpSettingsMock,
}));
...
interface HookSnapshot {
	...
	setMcpServers: (next: RuntimeKanbanMcpServer[]) => void;
...
function HookHarness({
	...
	const state = useRuntimeSettingsKanbanMcpController({
		open,
		workspaceId,
		selectedAgentId,
		liveAuthStatuses,
...
describe("useRuntimeSettingsKanbanMcpController", () => {
	let container: HTMLDivElement;
	let root: Root;
	let previousActEnvironment: boolean | undefined;

	beforeEach(() => {
		fetchKanbanMcpSettingsMock.mockReset();
		fetchKanbanMcpAuthStatusesMock.mockReset();
		runKanbanMcpServerOAuthMock.mockReset();
		saveKanbanMcpSettingsMock.mockReset();
		fetchKanbanMcpSettingsMock.mockResolvedValue({
			path: "/tmp/cline_mcp_settings.json",
			servers: [],
		});
		fetchKanbanMcpAuthStatusesMock.mockResolvedValue({
			statuses: [],
		});
		runKanbanMcpServerOAuthMock.mockResolvedValue({
			serverName: "linear",
			authorized: true,
			message: "Authorized",
		});
		saveKanbanMcpSettingsMock.mockResolvedValue({
			path: "/tmp/cline_mcp_settings.json",
			servers: [],
		});
	...
	it("loads MCP settings when Kanban is selected", async () => {
		...
		expect(fetchKanbanMcpSettingsMock).toHaveBeenCalledWith("workspace-1");
		expect(fetchKanbanMcpAuthStatusesMock).toHaveBeenCalledWith("workspace-1");
	...
	it("tracks unsaved MCP changes and persists them", async () => {
		...
		expect(saveKanbanMcpSettingsMock).toHaveBeenCalledWith(null, {
			servers: [
				{
					name: "linear",
					disabled: false,
		...
		expect(fetchKanbanMcpAuthStatusesMock).toHaveBeenCalledWith(null);
		expect(requireSnapshot(latestSnapshot).hasUnsavedChanges).toBe(false);
	});
	...
	it("runs MCP OAuth and refreshes auth statuses", async () => {
		...
		fetchKanbanMcpAuthStatusesMock
		...
		expect(runKanbanMcpServerOAuthMock).toHaveBeenCalledWith("workspace-1", {
			serverName: "linear",
		});
		expect(fetchKanbanMcpAuthStatusesMock).toHaveBeenCalledTimes(2);
	...
	it("applies live auth status updates while OAuth is still in progress", async () => {
		let latestSnapshot: HookSnapshot | null = null;
		let liveAuthStatuses: RuntimeKanbanMcpServerAuthStatus[] | null = null;
		let resolveOauth: (() => void) | null = null;
		runKanbanMcpServerOAuthMock.mockImplementation(
			() =>
				new Promise((resolve) => {
					resolveOauth = () => {
						resolve({
	...
	it("saves unsaved MCP settings before running OAuth", async () => {
		...
		expect(saveKanbanMcpSettingsMock).toHaveBeenCalledBefore(runKanbanMcpServerOAuthMock);
		expect(saveKanbanMcpSettingsMock).toHaveBeenCalledWith("workspace-1", {
			servers: [
				{
					name: "linear",
					disabled: false,
	...
	it("does not load MCP settings when a non-Kanban agent is selected", async () => {
		let latestSnapshot: HookSnapshot | null = null;
		...
		expect(fetchKanbanMcpSettingsMock).not.toHaveBeenCalled();
		expect(fetchKanbanMcpAuthStatusesMock).not.toHaveBeenCalled();

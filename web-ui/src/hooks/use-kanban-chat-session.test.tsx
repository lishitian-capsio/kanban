...
import { type KanbanChatMessage, useKanbanChatSession } from "@/hooks/use-kanban-chat-session";
...
function HookHarness({
	...
	onSendMessage?: (
		...
		text: string,
		options?: { mode?: RuntimeTaskSessionMode; images?: RuntimeTaskImage[] },
	) => Promise<{ ok: boolean; message?: string; chatMessage?: KanbanChatMessage | null }>;
	...
	const state = useKanbanChatSession({
		taskId,
		onSendMessage,
		onLoadMessages,
		incomingMessages,
...
describe("useKanbanChatSession", () => {
	...
	it("appends incoming stream messages and updates existing messages by id", async () => {
		const initialMessage: KanbanChatMessage = {
			id: "initial",
			role: "assistant",
			content: "Initial response",
			createdAt: 1,
		};
		const streamedMessage: KanbanChatMessage = {
			id: "streamed",
			role: "assistant",
		...
		const streamedMessageUpdate: KanbanChatMessage = {
			...streamedMessage,
	...
	it("updates existing message when only meta changes", async () => {
		const toolStart: KanbanChatMessage = {
			id: "tool-1",
			role: "tool",
			content: "Tool: Read",
			createdAt: 2,
	...
	it("clears local messages when incomingMessages transitions to an explicit empty array", async () => {
		const loadedMessages: KanbanChatMessage[] = [
			{
				id: "loaded-1",
				role: "user",
				content: "Hello",

// Shared TRPC action hook for every native Kanban chat surface.
...
export interface KanbanChatActionResult {
	ok: boolean;
	message?: string;
...
interface UseKanbanChatRuntimeActionsResult {
	sendTaskChatMessage: (
		taskId: string,
		text: string,
		options?: SendKanbanChatMessageOptions,
...
export function useKanbanChatRuntimeActions({
	currentProjectId,
	onSessionSummary,
}: UseKanbanChatRuntimeActionsInput): UseKanbanChatRuntimeActionsResult {
	const sendTaskChatMessage = useCallback(
		async (taskId: string, text: string, options?: SendKanbanChatMessageOptions): Promise<KanbanChatActionResult> => {
			if (!currentProjectId) {
	...
	const abortTaskChatTurn = useCallback(
		async (taskId: string): Promise<KanbanChatActionResult> => {
			if (!currentProjectId) {
				return { ok: false, message: "No project selected." };
			}
			try {

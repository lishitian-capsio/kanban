...
const SETTINGS_AGENT_ORDER: readonly RuntimeAgentId[] = ["pi", "claude", "codex", "droid", "kiro", "qoder"];
...
export function RuntimeSettingsDialog({
	...
	// After saving an agent's executable-path override (in the General → Agent
	// subsection), reload the provider sets and refresh the runtime config so the
	// agent's "installed" badge reflects the override that was just applied.
	const handleExecutablePathSaved = useCallback(() => {
		reloadProviderSets();
		refreshConfig();
	}, [refreshConfig, reloadProviderSets]);
					...
					<div className="rounded-lg border border-border bg-surface-0 px-4 py-3 mb-4">
						...
						{displayedAgents.map((agent) => (
							<div key={agent.id}>
								...
								/>
								{/* pi launches in-process, so the executable-path override is offered
								    for external CLI agents only. */}
								{agent.id !== "pi" ? (
									<AgentExecutablePathSetting
										agentId={agent.id}
										workspaceId={workspaceId}
										persistedExecutablePath={providerSetsByAgent[agent.id]?.executablePath ?? ""}
										installed={agent.installed}
										disabled={controlsDisabled}
										onSaved={handleExecutablePathSaved}
									/>
								) : null}
							</div>
						))}

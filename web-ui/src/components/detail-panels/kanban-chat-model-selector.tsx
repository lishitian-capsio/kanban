...
import type { RuntimeReasoningEffort } from "@/runtime/types";
...
export function KanbanChatModelSelector({
	modelOptions,
	recommendedModelIds = [],
									...
									{CLINE_REASONING_EFFORT_OPTIONS.map((option) => {
										...
										return (
											<button
												...
												onClick={() => {
													onSelectReasoningEffort(option.value as RuntimeReasoningEffort | "");
													handleOpenChange(false);
												}}
											>
												<span className="flex-1">
													{formatKanbanReasoningEffortLabel(
														option.value as RuntimeReasoningEffort | "",
													)}
												</span>
												{isSelected ? <Check size={14} className="shrink-0 text-accent-fg" /> : null}
											</button>

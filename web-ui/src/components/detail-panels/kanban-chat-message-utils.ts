import { type KanbanToolCallDisplay, getKanbanToolCallDisplay } from "@runtime-kanban-tool-call-display";
 ...
 * Uses tool-specific logic for known Kanban SDK tools, then falls back to generic extraction.
 */
export function getToolSummary(toolName: string, input: string | null): string | null {
	return getKanbanToolCallDisplay(toolName, input).inputSummary;
}
 ...
 */
export function getToolDisplay(toolName: string, input: string | null): KanbanToolCallDisplay {
	return getKanbanToolCallDisplay(toolName, input);
}

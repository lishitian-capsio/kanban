// The fullscreen Task tab: a lean, read-only tracker of the actively executing
// tasks (decision e3134 / fullscreen-task-tab-direction).
//
...
// no columns, no card grid, no drag, no write actions. It shows only the active
// tasks — the `in_progress` column (who's running) above the `review` column
// (who's waiting on me) — each as one compact row reusing the SAME status-dot
// semantics and live agent-activity line as the fullscreen session cards
// (home-session-card-derive / session-activity). Clicking a row jumps into that
// task's detail/transcript via `onOpenTask` (the caller exits fullscreen so the
// task detail view — which renders below the fullscreen overlay — becomes visible).
//
	...
	return (
		<div className="flex min-h-0 flex-1 flex-col">
			...
			<div className="min-h-0 flex-1 overflow-y-auto px-1 pb-2">
				{entries.length === 0 ? (
					<ActiveTaskEmptyState />
				) : (
					<ul className="flex flex-col gap-1.5">
						{entries.map((entry) => (
							<ActiveTaskRow key={entry.taskId} entry={entry} agents={agents} onOpenTask={onOpenTask} />
						))}
					</ul>
				)}
			</div>
...
function ActiveTaskEmptyState(): ReactElement {
	return (
		<div className="flex h-full flex-col items-center justify-center gap-2 px-6 text-center">
			...
			<p className="m-0 text-sm font-medium text-text-secondary">No active tasks</p>
			<p className="m-0 max-w-[260px] text-[12px] leading-snug text-text-tertiary">
				Tasks in progress and awaiting review show up here while their agents run.
			</p>
		</div>
	);
}

const COLUMN_LABEL: Record<ActiveTaskEntry["columnId"], string> = {
	...
	return (
		<li>
			<button
				...
				className="group flex w-full cursor-pointer items-start gap-2.5 rounded-md border border-border bg-surface-2 px-3 py-2.5 text-left transition-colors hover:border-border-bright hover:bg-surface-3 focus:outline-none focus-visible:border-border-focus"
			>
				<span

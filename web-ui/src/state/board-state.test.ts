...
describe("board dependency state", () => {
	...
	it("preserves task-level pi overrides when updating the title", () => {
		let board = createInitialBoardData();
		board = addTaskToColumn(board, "backlog", {
			prompt: "Task with pi overrides",
			agentId: "pi",
			agentSettings: {
		...
		const updatedTask = updated.board.columns.find((column) => column.id === "backlog")?.cards[0];
		expect(updatedTask?.title).toBe("Updated title");
		expect(updatedTask?.agentId).toBe("pi");
	...
	it("does not treat non-pi agent overrides as explicit pi settings", () => {
		let board = createInitialBoardData();
		board = addTaskToColumn(board, "backlog", {
	...
	it("materializes a concrete kanban override when saving task-level chat settings", () => {
		let board = createInitialBoardData();
		board = addTaskToColumn(board, "backlog", {
	...
	it("keeps tasks pinned to pi when the global selected agent is different", () => {
		let board = createInitialBoardData();
		board = addTaskToColumn(board, "backlog", {
			prompt: "Task pinned to pi",
			agentId: "pi",
			agentSettings: {
				providerId: "openrouter",
				modelId: "anthropic/claude-sonnet-4.6",

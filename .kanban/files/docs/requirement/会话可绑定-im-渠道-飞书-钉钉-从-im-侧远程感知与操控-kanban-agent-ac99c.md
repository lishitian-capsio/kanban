...
- **阶段 2 — New IM thread（双向遥控）**：类型 B 独立创建入口，随入站双向能力一起上线。架构路线已由同类开源实现（OpenClaw / Nous Research 的 hermes-agent，MIT）验证：**轻量常驻 gateway + 长连接入站 + pairing-code 绑定 + agent 按需唤醒**（见交付注记）。
...
- 入站（阶段 2 / 类型 B）架构路线（参考 OpenClaw / hermes-agent 已验证）：
  ...
  - **pairing-code 绑定**（替代下拉选群）：用户在 IM 里私聊 / @ bot → bot 回一个一次性配对码 → 用户在 Kanban 输入码完成批准 → 绑定该 IM 会话 ↔ thread。**非 OAuth**，走长连接，无回调；建立绑定的同时即打通入站。
  - gateway 收到事件后按 chatId→threadId 路由投递（与出站的 originThreadId→imChannel 路由对称）。

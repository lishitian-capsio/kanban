...
import type { RuntimeKanbanMcpServer, RuntimeKanbanMcpSettingsResponse } from "../../core/api-contract";
...
function normalizeServer(server: RuntimeKanbanMcpServer): RuntimeKanbanMcpServer {
...
function normalizeServers(servers: RuntimeKanbanMcpServer[]): RuntimeKanbanMcpServer[] {
...
export function resolveMcpSettingsPath(): string {
	const configuredPath = process.env.KANBAN_MCP_SETTINGS_PATH?.trim();
	if (configuredPath) {
		return resolve(configuredPath);
	}
	return join(homedir(), ".kanban", "settings", "mcp_settings.json");
}

function parseSettingsFile(filePath: string): RuntimeKanbanMcpServer[] {
	...
	const servers = Object.entries(parsed.data.mcpServers).map(([name, raw]): RuntimeKanbanMcpServer => {
		if ("command" in raw) {
			return {
				name,
...
export interface KanbanMcpSettingsService {
	loadSettings(): RuntimeKanbanMcpSettingsResponse;
	saveSettings(input: { servers: RuntimeKanbanMcpServer[] }): Promise<RuntimeKanbanMcpSettingsResponse>;
}

export function createKanbanMcpSettingsService(): KanbanMcpSettingsService {
	return {
		loadSettings(): RuntimeKanbanMcpSettingsResponse {
		...
		async saveSettings(input: { servers: RuntimeKanbanMcpServer[] }): Promise<RuntimeKanbanMcpSettingsResponse> {
			const path = resolveMcpSettingsPath();

// Pure state helpers for task session management.
// Extracted so the pi adaptation layer has no dependency on external
// agent SDK internals.
//
// This module owns the in-memory summary and message shape plus the low-level
// mutations shared by the event adapter and the task session service.
...
export interface KanbanTaskSessionEntry {
...
export function cloneMessage(message: KanbanTaskMessage): KanbanTaskMessage {
	return {
		...message,
...
export function latestAssistantMessageMatches(entry: KanbanTaskSessionEntry, content: string): boolean {
...
export function clearActiveTurnState(entry: KanbanTaskSessionEntry): void {
...
export function appendAssistantChunk(entry: KanbanTaskSessionEntry, taskId: string, chunk: string): KanbanTaskMessage {
...
export function appendReasoningChunk(entry: KanbanTaskSessionEntry, taskId: string, chunk: string): KanbanTaskMessage {
...
function updateMessageInEntry(
	entry: KanbanTaskSessionEntry,
	messageId: string,
	updater: (currentMessage: KanbanTaskMessage) => KanbanTaskMessage,
): KanbanTaskMessage | null {
...
function getLatestAssistantMessage(entry: KanbanTaskSessionEntry): KanbanTaskMessage | null {

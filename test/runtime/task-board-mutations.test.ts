...
describe("task origin", () => {
	it("stores the home-thread origin supplied at creation", () => {
		const created = addTaskToColumn(
			createBoard(),
			"backlog",
			{ prompt: "Task", baseRef: "main", origin: { agentId: "pi", threadId: "thread-abc" } },
			() => "aaaaa111",
		);
		expect(created.task.origin).toEqual({ agentId: "pi", threadId: "thread-abc" });
	});

	it("leaves origin undefined when none is supplied", () => {
		const created = addTaskToColumn(createBoard(), "backlog", { prompt: "Task", baseRef: "main" }, () => "aaaaa111");
		expect(created.task.origin).toBeUndefined();
	});

	it("copies the origin so the card does not retain the caller's reference", () => {
		const origin = { agentId: "pi" as const, threadId: "thread-abc" };
		const created = addTaskToColumn(
			createBoard(),
			"backlog",
			{ prompt: "Task", baseRef: "main", origin },
			() => "aaaaa111",
		);
		expect(created.task.origin).not.toBe(origin);
		expect(created.task.origin).toEqual(origin);
	});

	it("preserves origin across move operations", () => {
		const created = addTaskToColumn(
			createBoard(),
			"backlog",
			{ prompt: "Task", baseRef: "main", origin: { agentId: "claude", threadId: "default" } },
			() => "aaaaa111",
		);
		const moved = moveTaskToColumn(created.board, created.task.id, "in_progress");
		expect(moved.task?.origin).toEqual({ agentId: "claude", threadId: "default" });
	});
});

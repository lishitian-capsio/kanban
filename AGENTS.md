...
- Kanban's native agent layer supports both `pi` (oh-my-pi, source-embedded in `src/agent-sdk/`) and `cline` (via `src/cline-sdk/` boundary + `@clinebot/*` packages). The runtime uses a dual-path design: `runtime-api.ts` and `runtime-server.ts` manage both service types, preferring pi when available. The pi adaptation layer (`src/agent-sdk/kanban/`) maps omp `AgentEvent` to Kanban's `RuntimeTaskSessionSummary` contract.
...
- Commander nested subcommands: if a child subcommand re-declares an option already declared on its parent (e.g. `--project-path` on both `requirement review` and `requirement review apply`), commander silently routes the passed value to the *parent* command, so the child action's plain `options` arg has it `undefined`. Read it via `this.optsWithGlobals()` inside the child action (use a regular `function`, not an arrow, so `this` is the Command). This bit during requirement-review wiring and looked like a workspace-resolution bug.
